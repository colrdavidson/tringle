...
static int memfd_create(char *filename, unsigned int flags) {
	return syscall(SYS_memfd_create, filename, flags);
}

static int execve(char *pathname, char **argc, char **argv) {
	return syscall(SYS_execve, pathname, argc, argv);
}
...
static void print(int fd, char *c) {
	uint8_t obuf[128];
